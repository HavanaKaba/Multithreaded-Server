...
char * winsock_server::receive_message(int index) {
	recv(this->connection_socket[index], this->rx_buffer, sizeof(this->rx_buffer) + 1, 0);
...
void winsock_server::rec_mode(int socket_number) {
	while (true) {
		...
		message_sender = socket_number;
		buffer_full = true;
		std::string s = "User " + std::to_string(message_sender) + ": " + message_buffer;
		Print(s);
		if (strcmp(message_buffer, "quit") == 0) {
			s = "Close connection with user " + std::to_string(message_sender);
			Print(s);
			closesocket(this->connection_socket[socket_number]);
			this->active_sockets[socket_number] = 0;
			break;
		...
		if (buffer_full) {
			for (int socket_number = 0; socket_number < MAX_SOCKETS; socket_number++) {
				if (this->active_sockets[socket_number] == 1)
					send_message(message_buffer, socket_number);
			}
			buffer_full = false;
			if (strcmp(message_buffer, "quit") == 0) {
				std::string s = "Close connection with user " + std::to_string(message_sender);
				Print(s);
...
// infinite loop that calls and prints receive message method
